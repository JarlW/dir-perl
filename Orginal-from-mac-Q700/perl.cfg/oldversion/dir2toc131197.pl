...
#                    Fick recursionen att fungera!!!(pŒ Mac)
...
sub doTemp{
	...
	local ($a);
	local ($b);
	if ($OS=~"MAC"){$a ="$dirSep$currDir";}	    #MACOS
	if ($OS=~"WIN")	{$a ="$currDir";}			#WINDOWS
	if ($OS=~"UNIX"){$a ="$currDir";}			#UNIX	



	@path = split(/$dirSep/,$currDir);
		for (@path){
				$b="";
			 $b ="../$_/$tocFile";
			#print  "$b\n";
...
#open a toc.file in curr dir
	...
	print TOCH "<B><A HREF=\"\.\.\/$tocFile\" TARGET=\"$tocTarget\">HOME</A>\<\/B\>\<BR\>";	
...
sub startHtml {
	local($in) = @_;
	local($style) ="";
	open (STYLE, "$rootDir$dirSep"."$cfgDir$dirSep"."defaultstyle\.cfg");
	while (<STYLE>){
		$style="$style$_";
	}
